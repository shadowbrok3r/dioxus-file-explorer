            ...
            if *app_view.read() == AppView::DebugDb {
                ...
                div {
                    ...
                    section {
                        ...
                        // Debug: surface raw results length so we can see if signal updates propagate to UI
                        {
                            let len_now = results.read().items.len();
                            log::warn!("[ui-debug] render gate results_len={len_now} ai_active={} search='{}'", *ai_search_active.read(), search_text.read());
                            rsx! { p { class: "text-[10px] text-amber-600 mb-1", "[debug] results.len = {len_now}" } }
                        }
