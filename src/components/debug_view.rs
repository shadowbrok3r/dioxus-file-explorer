...
pub fn DebugView(props: DebugViewProps) -> Element {
    ...
    rsx! { div { class: "p-4 overflow-y-auto", style: "height: calc(100vh - 56px);",
        ...
        div { class: "flex flex-wrap gap-4 mb-6 text-sm",
            span { class: "px-2 py-1 bg-muted rounded border border-stroke", "Thumb rows: {thumbs.read().len()}" }
            ...
            button { class: "btn", onclick: move |_| {
                if let Some(engine_inst) = engine.read().as_ref() {
                    ...
                    spawn(async move {
                        ...
                        thumb_sig.set(t); doc_sig.set(d); ts_sig.set(Some(std::time::Instant::now()));
                    });
                }
        ...
        if thumbs.read().is_empty() { p { class: "text-weak text-sm", "No rows." } }
        else {
            { let rows = thumbs.read().clone(); rsx! {
                    ...
                    table { class: "min-w-full text-11px",
                        ...
                        tbody {
                            for row in rows.iter() {
                                { let row_clone = row.clone(); let path_for_click = row_clone.path.clone(); rsx! {
                                    tr { class: "border-b border-stroke hover:bg-accent-weak/40 cursor-pointer", onclick: move |_| { selected_path.set(Some(std::path::PathBuf::from(path_for_click.clone()))); },
                                        td { class: "pr-2 py-1 max-w-[240px] truncate", title: "{row_clone.path}", "{row_clone.filename}" }
                                        td { "{row_clone.file_type}" }
                                        td { "{row_clone.size}" }
                                        td { if row_clone.description.as_ref().map(|d| d.len()).unwrap_or(0) > 0 { "Y" } else { "" } }
                                        td { class: "truncate max-w-[140px]", title: row_clone.caption.clone().unwrap_or_default(), {row_clone.caption.clone().unwrap_or_default()} }
                                        td { class: "truncate max-w-[160px]", title: row_clone.tags.join(", "), "{row_clone.tags.len()}" }
                                        td { class: "truncate max-w-[140px]", { row_clone.hash.clone().unwrap_or_default().chars().take(12).collect::<String>() } }
                                    }
                                }}
                            }
                        }
        ...
        if docs.read().is_empty() { p { class: "text-weak text-sm", "No documents (index not built)." } }
        else {
            ul { class: "space-y-2 text-11px",
                for d in docs.read().iter() {
                    { let d_clone = d.clone(); rsx! {
                        li { class: "p-2 rounded border border-stroke bg-panel hover:border-accent transition",
                            { let title = d_clone.title.clone().unwrap_or_else(|| "(no title)".into()); rsx! { div { class: "flex justify-between text-10px text-weak mb-1", span { "{title}" } span { "{d_clone.len} chars" } } } }
