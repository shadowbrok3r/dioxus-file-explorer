...
pub fn PreviewPane(props: PreviewPaneProps) -> Element {
    ...
    rsx! {
        aside {
            class: "bg-panel border border-stroke p-2",
            style: "{style}",
                                                                        ...
                                                                        if let Some(disp) = disp_opt {
                                                                            if disp.has_text {
                                                                                div { class: "flex flex-col gap-1",
                                                                                    ...
                                                                                    if disp.long {
                                                                                        button {
                                                                                            class: "popup-menu-header self-start text-10px px-2 py-0.5 rounded bg-panel border border-stroke hover:border-accent transition",
                                                                                            onclick: move |_| {
                                        ...
                                        .find(|f| f.path == selected.display().to_string())
                                    {
                                        div { class: "space-y-2 text-sm",
                                            if let Some(desc) = ai_item.description.clone() {
                                                div { class: "p-2 rounded-md bg-muted border border-stroke text-11px leading-snug",
