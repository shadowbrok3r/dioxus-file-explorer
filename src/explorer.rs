...
pub fn list_dir_items(root: PathBuf) -> Result<Vec<DirItem>, String> {
    ...
    for e in rd.flatten() {
        let p = e.path();
        if let Ok(ft) = e.file_type() { if ft.is_dir() { out.push(DirItem { path: p }); } }
    }
...
use windows::{
    core::PCWSTR,
    Win32::Storage::FileSystem::{GetDriveTypeW, GetLogicalDrives, GetDiskFreeSpaceExW, GetVolumeInformationW},
};
#[cfg(windows)]
    ...
    unsafe {
        ...
        for i in 0..26u32 {
            if (mask & (1 << i)) != 0 {
                ...
                let nul = vol_name.iter().position(|&c| c == 0).unwrap_or(vol_name.len());
                let label = String::from_utf16_lossy(&vol_name[..nul]).trim().to_string();

                // Free/Total
                let mut free_avail: u64 = 0;
                let mut total: u64 = 0;
                let mut total_free: u64 = 0;
                let _ = GetDiskFreeSpaceExW(PCWSTR(wide.as_ptr()), Some(&mut free_avail), Some(&mut total), Some(&mut total_free));

                if total > 0 { out.push(DriveInfo { root, label, free: total_free, total }); }
            }
        }
        out
    }
...
#[cfg(not(windows))]
pub fn list_drive_infos() -> Vec<DriveInfo> { Vec::new() }
...
#[cfg(not(windows))]
pub fn drive_icon_for_root(_root: &str) -> &'static str { "storage" }
