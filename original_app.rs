...
use crate::explorer::{
    default_pictures_root, drive_icon_for_root, list_dir_items, list_drive_infos, quick_access,
};
use crate::scan::{ScanMsg, begin_scan};
use crate::settings::{SortBy, SortSetting, load_settings, save_settings};
...
pub fn app() -> Element {
    ...
    let mut sort = use_signal(|| {
        ui.read().sort.clone().unwrap_or(SortSetting {
            by: SortBy::Name,
            asc: true,
        })
    });
    ...
    let mut view_mode = use_signal(|| match ui.read().view_mode.as_deref() {
        Some("icons") => ViewMode::Icons,
        _ => ViewMode::Details,
    });
    ...
    let mut ai_search_results = use_signal(|| Vec::<crate::ai::FileMetadata>::new());
    ...
    if let Some(rx) = rx_state.read().as_ref() {
        for msg in rx.try_iter() {
            match msg {
                ...
                ScanMsg::Done => {
                    scanning.set(false);
                    scan_finished.set(Some(std::time::Instant::now()));
                }
            }
        }
    }
    ...
    let left_w: String = if *qa_collapsed.read() && *drives_collapsed.read() {
        "0px".to_string()
    } else {
        format!("{}px", *left_width.read())
    };
    // Preview target width; when collapsed we override style to 0 with no padding/border
    let right_w: String = if *preview_collapsed.read() {
        "0px".to_string()
    } else {
        format!("{}px", *preview_width.read())
    };
    ...
    let preview_style: String = if *preview_collapsed.read() {
        ...
        format!(
            "width: {right_w}; overflow: hidden; transition: width .08s ease; position: relative;"
        )
    };
    ...
    let filtered_items = use_memo(move || {
        ...
        results
            ...
            .iter()
            .filter(|it| {
                if only_with_thumb && it.thumb_data.is_none() {
                    return false;
                }
                if let Some(ext) = it
                    .path
    ...
    // Initialize AI search engine once
    {
        ...
        use_effect(move || {
            if ai_init_started_flag.get() || ai_engine_sig.read().is_some() {
                return;
            }
            ...
            spawn(async move {
                match crate::ai::AISearchEngine::new().await {
                    Ok(engine) => {
                        ...
                        for p in paths.iter() {
                            indexed_sig.write().insert(p.clone());
                        }
                        if let Ok(files) = engine.get_all_files().await {
                            for f in files.iter() {
                                if let Some(desc) = &f.description {
                                    ai_desc_sig.write().insert(f.path.clone(), desc.clone());
                                }
                            }
                        }
                        let engine_clone = engine.clone();
                        spawn(async move {
                            if let Err(e) = engine_clone.ensure_vision_model().await {
                                log::warn!("Vision model warm-up failed: {}", e);
                            } else {
                                ai_model_ready.set(true);
                            }
                        });
    ...
    // After model ready, enqueue one-time enrichment & pending refresh
    {
        let ai_engine_sig = ai_search_engine.clone();
        use_effect(move || {
            if !*ai_model_ready.read() || ai_pending_refreshed_flag.get() {
                return;
            }
            if let Some(engine) = ai_engine_sig.read().as_ref() {
                ...
                spawn(async move {
                    ...
                    if produced > 0 {
                        log::info!("Enriched {} missing AI descriptions post warm-up", produced);
                    }
                });
            }
        });
            ...
            spawn(async move {
                if let Some(engine) = engine_opt {
                    for item in new_items.iter() {
                        ...
                        if indexed_sig.read().contains(&path_str) {
                            continue;
                        }
                            ...
                            Ok(_) => {
                                ...
                                if let Some(meta) = engine.get_file_metadata(&metadata.path).await {
                                    if let Some(desc) = meta.description {
                                        ...
                                        spawn(async move {
                                            if let Ok(Some(desc)) = engine_clone
                                                .generate_description_for_path(&path_clone, false)
                                                .await
                                            {
                                                ai_desc_sig2
                                                    .write()
                                                    .insert(path_clone.clone(), desc);
                                            }
                                        });
                                    }
                                }
...
pub fn app_export_csv(items: &[crate::types::FoundFile]) -> Result<(), String> {
    ...
    wtr.write_record(["path", "kind", "modified", "created", "size_bytes"])
        .map_err(|e| e.to_string())?;
    for it in items {
        let kind = match it.kind {
            crate::types::MediaKind::Image => "image",
            crate::types::MediaKind::Video => "video",
            crate::types::MediaKind::Other => "other",
        };
